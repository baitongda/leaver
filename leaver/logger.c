...
char *leaver_logger_message_replace(char *msg, size_t msg_len, zend_string *search, zend_string *replace)
{
    char *search_str;
    size_t search_len;
...
zend_string *leaver_logger_format_message(zend_string *message, zend_array *context)
{
    ...
    ZEND_HASH_FOREACH_STR_KEY_VAL(context, key, z_value) {
        if (key && 0 != strcasecmp(key->val, "exception")) {
            ...
            tmp = leaver_logger_message_replace(str, strlen(str), key, value);
            if (tmp) {
                efree(str);
                str = tmp;
            }
