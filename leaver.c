...
#include "leaver/appender/daily_file.h"

int leaver_call_method(zend_class_entry *obj_ce, zval *obj, const char *func_name, size_t func_name_len, uint32_t params_count, zval* params, zval *retval, zend_function **func)
{
    int result;
    zend_fcall_info fci;
    ...
    if (!func && !obj_ce) {
        ...
        if (!func || !*func) {
            if (NULL == (fcic.function_handler = zend_hash_find_ptr(func_table, Z_STR(fci.function_name)))) {
                // Core error.
                zend_error_noreturn(E_CORE_ERROR, "Couldn't find implementation for method %s%s%s", obj_ce ? ZSTR_VAL(obj_ce->name) : "", obj_ce ? "::" : "", func_name);
            }
...
PHP_MINIT_FUNCTION(leaver)
{
    ...
    LEAVER_CREATE(appender_dailyfile);
...
{
    // Request start time
    gettimeofday(&LEAVER_G(request_time), NULL);
